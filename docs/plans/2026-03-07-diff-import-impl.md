...
**Step 1: EstimateDiffInstallUseCase 実装**
...
type ImportCandidate struct {
	...
	Similarity  float64 // MinHash類似度（minhash時のみ）
}
...
func (u *EstimateDiffInstallUseCase) EstimateOne(ctx context.Context, filePath string) (*ImportCandidate, error) {
	...
	match, err := u.elsaRepo.FindMostSimilarByMinHash(ctx, sig.Bytes(), minhashThreshold)
	if err != nil {
		return nil, err
	}
	if match != nil {
		candidate.DestFolder = match.FolderPath
		candidate.MatchMethod = "minhash"
	...
	if err == nil && irResp != nil && irResp.Registered {
		...
		// 既存の導入先推定に委譲
		candidates, err := u.estimateUseCase.Execute(ctx, irResp.Title, irResp.Artist, parsed.MD5)
		if err == nil && len(candidates) > 0 {
			candidate.DestFolder = candidates[0].FolderPath
			candidate.MatchMethod = "ir"
			return candidate, nil
		}
	}

	// Step 3: タイトル・アーティスト一致
	title := parsed.Title
	artist := parsed.Artist
	if title != "" {
		candidates, err := u.estimateUseCase.Execute(ctx, title, artist, parsed.MD5)
		if err == nil && len(candidates) > 0 {
			candidate.DestFolder = candidates[0].FolderPath
			candidate.MatchMethod = "title"
			return candidate, nil
		}
	}
...
func (u *EstimateDiffInstallUseCase) saveIRResponse(ctx context.Context, md5 string, resp *port.IRResponse) {
	// 既存のUpsertChartMetaパターンに合わせてIR情報を保存
	now := time.Now()
...
- テーブルカラム: ファイル名, TITLE（+SUBTITLE）, ARTIST（+SUBARTIST）, 推定先, 推定方法, 操作（クリアボタン）
- 下部に「推定先に導入」ボタン
