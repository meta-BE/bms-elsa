...
非flexカラムは全て非resizable（`enableResizing: false`）とする。これにより：
- `columnSizing`に入るカラム = flexカラム = RESIZABLE_IDS（完全一致）
- afterUpdateのflex初期化で全resizableカラムがcolumnSizingに入る
- キー集合不一致バグが構造的に発生しない

### カラム並び替え

flexカラムを左側に集約し、eventNameをflex化。releaseYearはeventNameの直後に配置（ドメイン的に近接）。

**ChartListView:** title, artist, genre, eventName, releaseYear, bpm, notes, ir
**SongListView:** title, artist, genre, eventName, releaseYear, bpm, chartCount, ir
**DifficultyTableView:** level, title, artist, hasUrl, statusLabel（変更なし）
**DiffImportView:** fileName, title, artist, destFolder, score, matchMethod, actions（変更なし）

### リサイズ可否まとめ

| ビュー | flexカラム（resizable） | 非flexカラム（非resizable） |
|---|---|---|
| ChartListView | title, artist, genre, eventName | releaseYear, bpm, notes, ir |
| SongListView | title, artist, genre, eventName | releaseYear, bpm, chartCount, ir |
| DifficultyTableView | title, artist | level, hasUrl, statusLabel |
| DiffImportView | fileName, title, artist, destFolder | score, matchMethod, actions |
...
| `frontend/src/views/ChartListView.svelte` | 修正 | カラムリサイズ有効化・カラム並び替え |
| `frontend/src/views/SongListView.svelte` | 修正 | カラムリサイズ有効化・カラム並び替え |
| `frontend/src/views/DifficultyTableView.svelte` | 修正 | カラムリサイズ有効化 |
| `frontend/src/settings/Settings.svelte` | 修正 | テーブル別リセットボタン追加 |

---

## Task 1: Go側Config structの拡張とバインディング再生成
...
- [ ] **Step 3: Wailsバインディングを再生成**

Run: `cd /Users/yudai.kuroki/src/github.com/meta-BE/bms-elsa && wails generate`

`frontend/wailsjs/go/models.ts` の `Config` クラスに `columnWidths` フィールドが追加されることを確認。

- [ ] **Step 4: コミット**

```bash
git add app.go frontend/wailsjs/go/models.ts
...
columns定義の後、`$: importableCount` の前に以下を追加:
...
- [ ] **Step 2: 割合のメモリ保持用変数とリセットリスナー変数を追加**
...
- [ ] **Step 4: リサイズ完了時の保存関数とウィンドウリサイズ対応を追加**
...
- [ ] **Step 5: clearAll時にcurrentRatiosもリセット**

`clearAll` 関数（81-86行目）を修正:
...
svelte importに `afterUpdate`, `onDestroy` を追加:
...
- [ ] **Step 2: カラム定義を並び替え、非flexカラムにenableResizing: falseを設定**

columns定義（56-103行目）を以下に置き換え:
...
- [ ] **Step 5: 初期化・永続化・ウィンドウリサイズ・リセットのロジックを追加**
...
`<svelte:window on:keydown={handleKeyNav} />` を修正:

```svelte
<svelte:window on:keydown={handleKeyNav} on:resize={handleWindowResize} />
```

`<SortableHeader table={$table} />` を修正:
...
svelte importに `afterUpdate` を追加:
...
`let sorting: SortingState = []` の後に追加:
...
DifficultyTableViewは `writable` storeベースの`options`パターンでテーブルを管理している。この既存パターンに合わせてリサイズを組み込む。
...
- [ ] **Step 2: 非flexカラムにenableResizing: falseを設定**

`level` カラムに `enableResizing: false` を追加:
...
`hasUrl` カラムに `enableResizing: false` を追加:
...
`statusLabel` カラムに `enableResizing: false` を追加:
...
- [ ] **Step 6: 初期化・永続化・ウィンドウリサイズ・リセットのロジックを追加**
...
「ファイル別ログを出力」チェックボックスと「バックグラウンドタスク」dividerの間に追加:
...
1. カラムヘッダー間のリサイズハンドルがflexカラム間にのみ表示される
2. ドラッグでカラム幅を変更できる
3. 非flexカラム（releaseYear, bpm, notes, chartCount, ir, level, hasUrl, statusLabel, score, matchMethod, actions）のハンドルは非表示
4. ChartListView / SongListView のカラム順序が title, artist, genre, eventName, releaseYear, bpm, ... になっている
5. リサイズ後にアプリを再起動しても幅が維持されている
6. ウィンドウサイズを変更するとカラム幅が比例して再計算される
7. Settingsモーダルの「カラム幅をリセット」ボタンが機能し、即座にflex初期化に戻る
