...
- `bmssearch_id`: BMS Search exhibition ID（例: `"0FMNS6rxB5CIA8"`）。BMS Search由来でないレコードはNULL
- `official_name`: BMS Search上の正式名称（例: `"THE BMS OF FIGHTERS 2015 -Time Travelers-"`）
- `short_name`: 表示用の短縮名（例: `"BOF:TT"`）。初期値はofficial_nameのコピー、後から編集可能
...
```sql
SELECT sm.*, e.short_name AS event_name, e.official_name
FROM song_meta sm
...
6. `exhibition.id` があれば、eventテーブルの `bmssearch_id` と照合してevent_idを取得
...
2. `bmssearch_id` がeventテーブルに存在しないレコードをINSERT
3. `short_name` の初期値は `official_name` のコピー
4. 既存レコードは `official_name` のみ更新（short_nameはユーザー編集を尊重して上書きしない）
...
- イベント一覧表示（official_name, short_name, release_year）
- short_nameのインライン編集
...
1. **既存のevent_mappings.csv**: url_pattern, event_name, release_year → event_nameをshort_name、official_nameの初期値として使用。bmssearch_idは後でBMS Searchと突合して紐付け
2. **BMS Search `/exhibitions/search`**: 全イベントを取得し、bmssearch_id, official_name, release_yearを取得

マージルール:
- BMS SearchのイベントをベースとしてINSERT
- event_mappings.csvのevent_nameをshort_nameとして、BMS Searchのofficial_nameと手動で対応付け（スクリプトで生成後、手動確認）
