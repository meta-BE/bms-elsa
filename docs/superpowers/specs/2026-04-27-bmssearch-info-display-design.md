...
### フォールバック検索の正規化・スコアリング（暫定）

事前調査スパイクの結果を踏まえて最終確定する。実装前にこのセクションを更新する前提。

**クエリ構築（暫定）**

- `?title={title}&limit=20&orderBy=PUBLISHED&orderDirection=DESC`
- title は songdata.db の原文を最初に使用、ヒット0なら正規化版で再試行
- 末尾の `[...]` `(...)` `-...-` は正規化で剥離する場合あり（パターン収集後に決定）
...
これらのルール詳細・配点・最大スコアは事前調査結果で確定する。
